...
@interface MainWindowViewController ()<NSTableViewDelegate, NSTableViewDataSource, NSDraggingDestination,NSTextFieldDelegate>
...
/**
 dSYM路径
 */
@property (weak) IBOutlet NSTextField *dSYMPath;
...
- (NSMutableArray *)allDSYMFilePath {
    NSString *archivesPath = self.dSYMPath.stringValue;
    NSFileManager *fileManager = [NSFileManager defaultManager];
//    NSString *archivesPath = @"/Users/zhangbin/Desktop/0_/";
    ...
    for (NSURL *fileURL in enumerator) {
        ...
        if (([filename hasSuffix:@".xcarchive"]  || [filename hasSuffix:@".dSYM"]) && [isDirectory boolValue]){
...
- (IBAction)sureButtonClick:(NSButton *)sender {
    ...
    if ([self.dSYMPath.stringValue isEqualToString:@""]) {
       NSLog(@"未输入路径");
        return;
    }
